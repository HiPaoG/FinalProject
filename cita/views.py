        ...
        if formulario.is_valid():
            ...
            for medico_id in request.POST.getlist('medico'):
                for paciente_id in request.POST.getlist('paciente'):
                    ...
                    return redirect('cita.views.DetalleCita', pk=cita.pk)
    else:
        formulario = CitaForm()
...
def EliminarCita(request, pk):
    cita = get_object_or_404(Cita, pk=pk)
    cita.delete()
    return redirect('cita.views.ListaCita')
...
def NuevoPaciente(request):
    if request.method == "POST":
        ...
        if formulario.is_valid():
            ...
            paciente.save()
            messages.add_message(request, messages.SUCCESS, 'Datos guardados')
            return redirect('cita.views.DetallePaciente', pk=paciente.pk)
    else:
...
def EliminarPaciente(request, pk):
    paciente = get_object_or_404(Paciente, pk=pk)
    paciente.delete()
    return redirect('cita.views.ListaPaciente')
...
def NuevoMedico(request):
    if request.method == "POST":
        ...
        if formulario.is_valid():
            ...
            medico.save()
            messages.add_message(request, messages.SUCCESS, 'Datos guardados')
            return redirect('cita.views.DetalleMedico', pk=medico.pk)
    else:
